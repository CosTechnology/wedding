...
async function main() {
  ...
  let content = 'Lado,Família,Nome,Status\n';
  
  families.forEach(family => {
    const responses = rsvpMap[family.id] || {};
    const lado = family.side === 'raynara' ? 'Raynara' : 'Gabriel';
    
    family.members.forEach(member => {
      const status = responses[member] === 'confirmed' ? 'Confirmado' : (responses[member] === 'declined' ? 'Não vai' : 'Sem resposta');
      content += `${lado},${escapeCsv(family.familyName)},${escapeCsv(member)},${status}\n`;
    });
  });

  writeFileSync('./convidados.csv', content, 'utf-8');

  console.log('✅ Arquivo CSV gerado com sucesso!');
  console.log('   📄 convidados.csv');
}
